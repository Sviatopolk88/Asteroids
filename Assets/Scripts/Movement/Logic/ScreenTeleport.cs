...
{
    private float _borderY => Camera.main.orthographicSize + 0.2f;
    ...
    public void BorderCrossing(Transform movingObject)
    {
        ...
        else if (objectPosition.y > _borderY * 0.85f)
        {
            movingObject.position = new Vector2(objectPosition.x, 0.01f - _borderY);
        }
        else if (objectPosition.y < -_borderY)
        {
            movingObject.position = new Vector2(objectPosition.x, _borderY * 0.85f - 0.01f);
        }
    }
}
